...
headers = {
    'Host': 'm.weibo.cn',
    'Refers': 'http"//m.weibo.cn/u/2830678474',#这里是html的网页
...
#打开Mongo把字典数据传入
def create_mongodb(name):
    client = MongoClient(host='localhost', port=27017)
    db = client[name] #如果没有则创建一个weibo db
    collection = db[name] #创建表 如果没有则创建一个collection表
    return collection
