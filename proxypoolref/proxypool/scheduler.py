...
from setting import CYCLE_GETTER, CYCLE_TESTER, API_HOST, API_THREADED, API_PORT, ENABLE_SERVER, \
    ENABLE_GETTER, ENABLE_TESTER, IS_WINDOWS
from loguru import logger


#这里解释大概就是Windows不支持fork函数，所以一般都是新建一个全新的跟主进程不相关的进程，
#因此需要通过腌制和管道传递执行的代码等等，这个函数就是判断是否需要管道传递
if IS_WINDOWS:
...
if __name__ == '__main__':
    scheduler = Scheduler()
    scheduler.run_getter(CYCLE_GETTER)
    # scheduler.run_tester(CYCLE_TESTER)
